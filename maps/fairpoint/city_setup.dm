/datum/map/fairpoint/send_welcome()
	var/obj/effect/overmap/visitable/ship/fairpoint = SSshuttle.ship_by_type(/obj/effect/overmap/visitable/ship/fairpoint)

	var/welcome_text = "<center><font size = 3><b>[global.using_map.station_name]</b> Sensor Readings:</font><br>"
	welcome_text += "Report generated on [stationdate2text()] at [stationtime2text()]</center><br /><br />"
	welcome_text += "<hr>Current system:<br /><b>[fairpoint ? system_name() : "Unknown"]</b><br /><br>"

	if(fairpoint) //If the overmap is disabled, it's possible for there to be no fairpoint.
		var/list/space_things = list()
		welcome_text += "Current Coordinates:<br /><b>[fairpoint.x]:[fairpoint.y]</b><br /><br>"
		...
		for(var/obj/effect/overmap/visitable/O in space_things)
			...
			if(O.loc != fairpoint.loc)
				var/bearing = round(90 - Atan2(O.x - fairpoint.x, O.y - fairpoint.y),5) //fucking triangles how do they work
				if(bearing < 0)
					bearing += 360
